...
def validate_language(value):
    ...
    if value not in ['nb', 'nn']:
        raise argparse.ArgumentTypeError(
            "Language must be either 'nb' or 'nn'")
    return value
...
def convert_ndt_to_ud(input_file, language, output_file, report_file):
    ...
    run_command(["python", "utils/convert_morph.py",
                "-f", input_file, "-o", temp_file])

    # Add MISC annotation 'SpaceAfter=No'
    temp_out = f"{temp_dir}/02_udapy_spaceafter.conllu"
    run_command(["python", "utils/udapi_tools.py", "-i",
                temp_file, "-o", temp_out, "-p", "space"])
    temp_file = temp_out
    ...
    with open(report_file, 'w') as report:
        validation_process = subprocess.run(
            ["python", "tools/validate.py", "--max-err",
                "0", "--lang", "no", output_file],
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert NDT treebank to UD format')
    parser.add_argument('-i', '--input', required=True, help='Input NDT file')
    parser.add_argument('-l', '--language', required=True, type=validate_language,
                        help='Language (must be nb or nn)')
    parser.add_argument('-o', '--output', default='UD_output.conllu',
                        help='Output UD file (default: UD_output.conllu)')
    parser.add_argument('-r', '--report', default='validation-report.txt',
